...
public class QwCodeOuterFace implements OuterFace {
	...
	@Override
	public Response callResources(final Request request) throws IllegalArgumentException {
		if (null == request) {
	...
	public void callResources(HttpServletRequest request,
			HttpServletResponse response) throws IllegalArgumentException {
		throw new UnsupportedOperationException("Not implemented yet.");
	}

	
	protected ResponseImpl aggregateResponses(final Request request, final Response response) {
	 ...
	 */
	protected InputStream convertCodesToXmlInputStream(final Map<String, AggregatedCode> codeMap) {
		...
		try {
			...
			if (codeMap != null && codeMap.size() > 0) {
				...
				TreeSet<AggregatedCode> orderedCodes = new TreeSet<>(codeComparator);
			...
			//I don't know how we can get here, but throw the RuntimeException just in case, rather than eating it. 
			e.printStackTrace();
